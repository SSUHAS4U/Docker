...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchFoodItems = async () => {
      try {
        const response = await axios.get('http://localhost:8082/food/all'); // Adjust this URL as per your backend API
  ...
  const handleSubmitEdit = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:8082/food/${editForm.id}`,
        formData,
        {
      ...
      const updatedFoodItems = await axios.get('http://localhost:8082/food/all');
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8082/food/${itemToDelete.id}`);
  ...
  const handleSearch = async (event) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8082/food/search?name=${query}`);
