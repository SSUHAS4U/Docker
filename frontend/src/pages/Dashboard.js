...
export default function MiniDrawer() {
  ...
  useEffect(() => {
    ...
    if (!authToken || !email) {
      ...
      // Fetch user details
      axios
        .get(`http://localhost:8082/users?email=${email}`, {
          headers: {
        ...
        .then((response) => {
          if (response.data.user) {
            ...
            axios
              .get(`http://localhost:8082/imageconverter/${email}`, {
                headers: {
                  Authorization: `Bearer ${authToken}`,
                },
              })
      ...
      const fetchStats = async () => {
        try {
          const statsResponse = await axios.get(
            `http://localhost:8082/fooddiary/foodstats`, // Updated endpoint
      ...
      const fetchExerciseStats = async () => {
        try {
          const statsResponse = await axios.get(
            `http://localhost:8082/exercisediary/exercisestats`, // Updated endpoint for exercise stats
            { headers: { email } } // Pass only the email in headers
          );
