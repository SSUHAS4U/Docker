...
export default function ExerciseStatistics() {
    ...
    const fetchFoodItems = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8082/exercisediary/list/${formattedDate}`,
          {
            headers: {
              email: email,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8082/exercisediary/delete/${id}`, {
        headers: {
          email: email,
          Authorization: `Bearer ${authToken}`,
  ...
  const handleShowDetails = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:8082/exercisediary/details/${id}`,
        {
          headers: {
            email: email,
            Authorization: `Bearer ${authToken}`,
  ...
  const handleAddFood = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8082/exercisediary/add",
        adjustedFoodItem,
        { headers: { email } }
      );
    ...
    const fetchStats = async () => {
      try {
        ...
        const formattedDate = new Date(selectedDate.getTime() - selectedDate.getTimezoneOffset() * 60000)
          ...
          .split("T")[0];
        const response = await axios.get(
          `http://localhost:8082/exercisediary/stats/${formattedDate}`,
          { headers: { email } }
        );
  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8082/exercisediary/exercisestats/graph/${formattedDate}`,
          { headers: { email } }
        );
