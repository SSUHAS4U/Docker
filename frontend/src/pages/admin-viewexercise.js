...
const ExerciseDashboard = () => {
  ...
  useEffect(() => {
    const fetchFoodItems = async () => {
      try {
        const response = await axios.get('http://localhost:8082/exercise/all'); // Adjust this URL as per your backend API
  ...
  const handleSubmitEdit = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:8082/exercise/${editForm.id}`,
        formData,
        {
      ...
      const updatedFoodItems = await axios.get('http://localhost:8082/exercise/all');
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8082/exercise/${itemToDelete.id}`);
  ...
  const handleSearch = async (event) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8082/exercise/search?name=${query}`);
