...
const AdminViewRecom = () => {
    ...
    const fetchFoodItems = async (nutritionalLevel = null) => {
        try {
            const url = nutritionalLevel
                ? `http://localhost:8082/recommendation/all?nutritionalLevel=${nutritionalLevel}`
                : `http://localhost:8082/recommendation/all`;
    ...
    const handleSubmitEdit = async () => {
        try {
            ...
            await axios.put(
                `http://localhost:8082/recommendation/${editForm.id}`,
                formData,
                {
            ...
            const updatedFoodItems = await axios.get('http://localhost:8082/recommendation/all');
    ...
    const handleConfirmDelete = async () => {
        try {
            await axios.delete(`http://localhost:8082/recommendation/${itemToDelete.id}`);
